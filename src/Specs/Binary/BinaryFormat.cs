...
public static class BinaryFormat
{
    public static Program Load(Stream stream)
    {
        ...
        var grid = Jagged.Array<Colour>(width, height);
        ...
        var colours = new Colours(grid);
        return Program.New(colours);
    }
    ...
    public static void Save(this Program program, Stream stream)
    {
        ...
        for (var x = 0; x < program.Width; x++)
        {
            ...
            {
                var colour = program[new(x, y)].Colour;

                size++;
                buffer *= 20;
                buffer += (ulong)colour.Index;
